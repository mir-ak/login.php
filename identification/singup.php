  ...
  <body>
    <?php
      ...
      if (isset($_REQUEST['username'], $_REQUEST['email'], $_REQUEST['password'])){
        ...
        $to_verify = "SELECT * FROM `Users_NXM8` WHERE username='".$username."'";
        $result = mysqli_query($conn,$to_verify) or die(mysql_error());
        $user = mysqli_num_rows($result);
        $to_verifyEmail = "SELECT * FROM `Users_NXM8` WHERE email='".$email."'";
        ...
        elseif(!preg_match($pattern,$password)){
          ...
          $query = "INSERT into `Users_NXM8` (username, email, password)
